...
az group create --name kantin-kenanga-rg --location eastus

# Create App Service Plan
az appservice plan create \
  --name kantin-kenanga-plan \
  --resource-group kantin-kenanga-rg \
  --sku B1 \
  --is-linux
...
az webapp create \
  --resource-group kantin-kenanga-rg \
  --plan kantin-kenanga-plan \
  --name kantin-kenanga-app \
  --runtime "PHP|8.2"
```
...
az webapp config appsettings set \
  ...
  --settings \
    ...
    APP_URL=https://kantin-kenanga-app.azurewebsites.net \
    DB_HOST=your-mysql-host.mysql.database.azure.com \
    DB_PORT=3306 \
    DB_DATABASE=improved_barnacle \
    DB_USERNAME=dbadmin@kantin-kenanga-db \
...
# Or via Azure CLI
az webapp log deployment show --name kantin-kenanga-app --resource-group kantin-kenanga-rg
```
...
# View logs
az webapp log tail --resource-group kantin-kenanga-rg --name kantin-kenanga-app
```
...
1. Check logs: `az webapp log tail --name kantin-kenanga-app --resource-group kantin-kenanga-rg`
